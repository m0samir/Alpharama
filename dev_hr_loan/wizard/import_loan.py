...
class import_loan(models.TransientModel):
    ...
    file_type = fields.Selection(
        [('excel', 'Excel'), ('csv', 'CSV')], string='File Type', default='csv')
    ...
    def get_loan_type(self, name):
        type_id = self.env['employee.loan.type'].search(
            [('name', '=', name)], limit=1)
        if type_id:
            return type_id
        else:
    ...
    def import_loan(self):
        ...
        if self.file_type == 'csv':
            ...
            lines = [[sheet.cell_value(r, c) for c in range(
        ...
        for line in lines:
            ...
            if not employee_id:
                ...
                if emp_loan >= employee_id.loan_request:
                    remark = self.add_in_remark(
                        remark, 'Employee Can not create more then ' + str(employee_id.loan_request))+' Loans'
